...
async def process_webhook(payload):
    ...
    # Two branches:
    #   A) Ticket already complete/done → post the closing note, skip status change
    #   B) Ticket not yet complete → post note + move to complete
    if gate == "CLOSURE" and not passed:
        ...
        if can_fix:
            ...
            print(f"[AGENT] Auto-fill triggered — soft gaps: {failing_checks} | already_complete={already_complete}", flush=True)
            ...
            if closing_note:
                ...
                if already_complete:
                    # Ticket is already done — just fill the gap, no status change needed
                    await post_comment(
                        task_id,
