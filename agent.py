...
6. DE Actionability — expected output clear, full BQ path provided, no TBDs. Actionable without a meeting. PASS AUTOMATICALLY when the ticket involves no data engineering work (no BQ query, no pipeline, no table build, no ingestion, no transformation, no SQL). NEW-BUILD EXCEPTION: If the ticket is explicitly creating a new BQ table, pipeline, or ingestion from scratch (signals: title or description contains "ingestion", "ingest", "new pipeline", "new connector", "new table", "build table", "create table", paths labeled "proposed" or "target", or description states the table "will be created" / "does not exist yet"), PASS if the BQ project and dataset are confirmed (e.g. project.dataset.anything) and a proposed/target table name following naming conventions is stated — the table does NOT need to exist yet. FAIL only when DE/data work is explicitly in scope AND absolutely no BQ project or dataset is mentioned.

INTAKE GATE — BI Tickets (use instead of Generic when BI ticket detected):

IMPORTANT — NEW-BUILD / INGESTION TICKETS: Before scoring check #3, decide if this is an ingestion or pipeline-build ticket. It IS one if: title or description contains "ingest", "ingestion", "pipeline", "connector", "new table", "build table" OR the description has a section titled "Target BQ paths" OR it describes creating raw/staging/presentation layers. If this IS a new-build/ingestion ticket: the upstream source (Northbeam, Shopify, etc.) is an external API — it has NO BQ path, and that is completely normal. For check #3, look instead at the TARGET BQ tables being created. PASS check #3 automatically if any full BQ path of the form project.dataset.table (wildcards like _data_* are fine) appears anywhere in the description. FAIL check #3 only if the description contains no BQ path at all.
...
3. Data source or target BQ path confirmed. For existing-source tickets: full source path (project.dataset.table) required. For new-build/ingestion tickets: see IMPORTANT override above — PASS if any target BQ path is present.
