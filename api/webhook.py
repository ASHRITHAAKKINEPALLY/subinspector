...
| # | Check | Result | Detail |
|---|-------|--------|--------|
| 1 | [Check name] | ✅ PASS or ❌ FAIL | [one-line detail] |
...
def evaluate_gate(gate, task):
    ...
    user_message = (
        ...
        f"Folder: {(task.get('folder') or {}).get('name', '')}\n"
        f"Description:\n{description}"
    )
...
def revert_status(task_id, status):
    with httpx.Client(timeout=15) as client:
        client.put(
            f"{CLICKUP_BASE}/task/{task_id}",
            headers={"Authorization": CLICKUP_API_KEY, "Content-Type": "application/json"},
...
def process_webhook(payload):
    ...
    if not gate:
        return
    content = evaluate_gate(gate, task)
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b'{"status": "SubInspector is running"}')
